...
const MentorshipRequestForm: React.FC<MentorshipRequestFormProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      loadFiltersData();
      if (initialMentor) {
        setSelectedMentore(initialMentor);
      }
    }
  }, [isOpen, initialMentor]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      console.log('Envoi demande:', { mentoreId: selectedMentore._id || selectedMentore.id, message: message.trim() });

      const response = await Api.post('/mentorship/request', {
        mentoreId: selectedMentore._id || selectedMentore.id,
                ...
                {mentores.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {mentores.map((mentore) => (
                      <div
                        key={mentore._id}
                        className={`p-4 border rounded-lg cursor-pointer transition-all ${selectedMentore?._id === mentore._id
